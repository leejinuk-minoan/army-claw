...
- 첨부 예시를 기준으로 좌측 내비게이션과 중앙 작업 피드 형태의 UI 구조를 추가했다.
- 실행 큐 화면에 전체/성공/건너뜀/실패/대기 집계와 생성 파일 경로를 표시하도록 개선했다.
- `HWP`, `.HWP`, `HWPX`, 한글/한컴 문서 요청이 `file` 단계로 분류되어도 HWPX 생성 스키마로 승격되도록 수정했다.
- 작업공간 경로가 비어 있으면 `%LOCALAPPDATA%\ArmyClaw\workspace`를 기본 출력 폴더로 사용하고 자동 생성하도록 수정했다.

### 이번 검증

- 실행 큐 단위 테스트 9개 통과.
- 전체 백엔드 테스트 71개 통과.
- React 프로덕션 빌드 통과.`n- `scripts\package-core.bat` 패키징 통과.`n- 패키지 EXE에서 작업공간 미지정 `.HWP` 요청이 기본 출력 폴더의 HWPX 파일로 생성되는 스모크 테스트 통과.
