...
9. adapter-related handoff라면 validator gate status를 확인한다.
10. stop condition이 하나라도 발생하면 작업을 중단하고 보고한다.
...
- validation summary
- adapter validator gate status and evidence path when required
...
- adapter validator gate required but missing or not passed
...
## 14. Adapter validator integration gate 확인 절차

Task 026 이후 adapter 관련 Task는 시작 전 다음을 확인한다.

1. `docs/architecture/army-claw-adapter-validator-integration-contract.md`를 읽는다.
2. `docs/gpt-communication/contracts/adapter-validator-integration-contract.json`을 확인한다.
3. `docs/gpt-communication/contracts/adapter-validator-gate-policy.json`을 확인한다.
4. `docs/gpt-communication/contracts/adapter-validator-evidence-schema.json`을 확인한다.
5. changed files 기준으로 `adapter_validator_gate_required`를 판정한다.
6. gate required인데 evidence가 없으면 completed 보고를 금지한다.
7. 로컬 evidence가 필요한 경우 cloud-first/local-verify로 분리한다.

## 15. Cloud-first local verification 절차

Task 025-A 이후 로컬 worker는 바로 실행하지 않는다.

1. 마스터가 원격 branch HEAD를 read-only로 검증한다.
2. 마스터가 `local_execution_base_sha`를 지정한다.
3. 로컬 worker는 지정 SHA에서 Task branch를 생성한다.
4. 로컬 worker는 승인된 명령만 실행한다.
5. stdout, stderr, exit code를 보고서에 기록한다.
6. 실패 시 허용된 최소 파일만 수정한다.

Cloud package 상태에서 `completion_gate_passed`는 false로 유지한다.
