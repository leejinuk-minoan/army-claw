...
def run_promotion(whole_manifest=True, request_mutator=None, manifest_mutator=None, probe=None, trusted_receipt=None, fixture=None):
    ...
    response = promote_staged_output(
        request,
        staged_root=staged,
        approved_roots={"approved-output": approved},
...
def scenario_idempotency(assertions):
    ...
    second = promote_staged_output(request, staged_root=staged, approved_roots={"approved-output": approved}, manifest_document=manifest, trusted_receipt=first["receipt"])
    try:
...
def scenario_preexisting(assertions):
    ...
    try:
        ...
        response = promote_staged_output(request, staged_root=staged, approved_roots={"approved-output": approved}, manifest_document=manifest)
...
def scenario_raw_root(assertions):
    ...
    for sid, probe, staged_override, approved_override, code in [
        ...
        try:
            ...
            response = promote_staged_output(request, staged_root=root if staged_override else staged, approved_roots={"approved-output": root if approved_override else approved}, manifest_document=manifest, filesystem_probe=probe(root))
    ...
    temp, staged, approved, source, request, manifest = base_fixture()
    try:
        response = promote_staged_output(request, staged_root=staged, approved_roots={"approved-output": approved}, manifest_document=manifest, filesystem_probe=FilesystemProbe(root_inspection_failure_paths={str(staged)}))
...
def scenario_lexical(assertions):
    ...
    for sid, probe, code in [
        ...
        temp, staged, approved, source, request, manifest = base_fixture()
        try:
            response = promote_staged_output(request, staged_root=staged, approved_roots={"approved-output": approved}, manifest_document=manifest, filesystem_probe=probe(staged, approved))
            assert_true(assertions, sid, response.get("error_code") == code, response)
            observed.append({sid: response.get("error_code")})
        finally:
            temp.cleanup()
...
def scenario_cross_volume(assertions):
    ...
    try:
        ...
        response = promote_staged_output(request, staged_root=staged, approved_roots={"approved-output": approved}, manifest_document=manifest, filesystem_probe=probe)
...
def scenario_structured(assertions):
    ...
    for sid, probe_factory, code in [
        ...
        temp, staged, approved, source, request, manifest = base_fixture()
        try:
            response = promote_staged_output(request, staged_root=staged, approved_roots={"approved-output": approved}, manifest_document=manifest, filesystem_probe=probe_factory(staged, approved, source))
            assert_true(assertions, sid, response.get("error_code") == code, response)
            observed.append({sid: response.get("error_code")})
        finally:
            temp.cleanup()
