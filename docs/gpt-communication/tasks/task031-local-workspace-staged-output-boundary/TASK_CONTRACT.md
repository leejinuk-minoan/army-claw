...
```text
task_id: task031-local-workspace-staged-output-boundary
title: Local Workspace Staged Output Boundary
...
Introduce a staged output boundary after Task 030 read-only manifest.

Task 031-A cloud package may update adapter source, tests, target-specific samples, architecture/contract documents, local verification package, report, Research Note, and status files.

Task 031-A must not claim final Task 031 completion.

## 3. Required behavior

Staged output mode must require explicit markers:
...
The adapter may write request-provided generated content only to a controlled temporary unit-test staging sandbox when local tests run.

The adapter must not:

```text
write into source workspace
overwrite source files
promote staged output to a final user workspace location
mutate real user workspace files
read real user workspace file contents
invoke native applications
invoke Hancom COM
generate real HWP/HWPX/HanCell/HanShow artifacts
claim actual adapter invocation
```
...
adapter_validator_evidence_path: docs/gpt-communication/evidence/task031-local-workspace-staged-output-boundary/
...
Reason: Task 031-A changes adapter code and tests, so local validator/unittest evidence is required.
...
docs/gpt-communication/contracts/samples/local-workspace-adapter/staged-output-*.json
...
docs/gpt-communication/delegation/task031-local-workspace-staged-output-boundary/*
...
```text
- do not create, modify, copy, delete, move, or mutate real user workspace files
- do not read real user workspace file contents
- do not implement production filesystem mutation
- do not promote staged output to a real user workspace destination
...
- do not modify Task 030 evidence
- do not modify Task 030 LOCAL_EXECUTION_RESULT.json
- do not modify Task 030 reports except by reference
...
- do not force push
- do not claim final Task 031 completion
...
Final Task 031 completion requires:
