...
- 논문화에 필요한 연구 질문, 설계 명제, 방법론, 결과, 한계는 `docs/research-notes/`의 Task별 Research Note로 분리해 관리한다.
...
8. 논문 활용 메모는 한 파일에 계속 누적하지 않고, `docs/research-notes/task-notes/`에 Task별 독립 파일로 작성한다.
9. Research Note의 번호와 위치는 `docs/research-notes/research-note-index.md`와 `docs/research-notes/research-note-index.json`에만 짧게 기록한다.
...
## Research Note와 보고서의 구분

```text
Task report
→ 작업 목적, 변경 파일, 테스트 결과, 증거 산출물, 제한사항을 기록하는 개발·검증 문서

Research Note
→ 논문 작성에 활용할 연구 질문, 설계 명제, 방법론, 검증 결과, paper-ready sentence, 한계를 재정리한 문서

Research Note Index
→ Research Note 번호와 파일 위치를 관리하는 짧은 색인
```

Codex 또는 다른 작업자는 Task 완료 시 보고서와 Research Note를 혼동해서는 안 된다. 보고서는 `docs/gpt-communication/reports/`에, 논문 활용 메모는 `docs/research-notes/task-notes/`에 둔다.
