...
- 현재 로컬 PC에 한컴오피스 2024가 설치되어 있어 한글/한셀/한쇼 네이티브 검증을 진행할 수 있다.
...
## 2026-06-28 현재 구현 업데이트

### Skill 관리 MVP

- v0.1에 Skill 업로드/관리 MVP가 추가되었다.
- 지원 형식은 `.zip` 패키지 안에 `SKILL.md`가 포함된 구조이다.
- 등록된 skill은 `%LOCALAPPDATA%\ArmyClaw\skills`에 저장된다.
- UI에서 업로드, 목록 확인, 활성/비활성 전환, 삭제가 가능하다.
- 각 skill은 SHA256 해시, 등록일, 상태, 원본 파일명을 메타데이터로 기록한다.
- 현재 단계의 skill은 모델 학습 데이터가 아니라 작업 계획과 프롬프트에 주입할 수 있는 로컬 지식 패키지이다.

### 다음 구현 목표

- 활성화된 skill을 실제 작업 계획 생성 또는 LLM 프롬프트 컨텍스트에 주입한다.
- skill 적용 이력을 작업 로그에 남긴다.
- v0.2에서는 반입 전 보안 검증과 skill 검색/추천을 강화한다.
