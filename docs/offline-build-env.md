...
- `vendor/python-wheels.sha256`: wheelhouse 무결성 확인용 해시 매니페스트. GitHub에는 올리지 않는다.
...
.\scripts\write-wheelhouse-manifest.bat
```

3. 생성된 `vendor/python-wheels/` 폴더와 `vendor/python-wheels.sha256` 파일을 보안 검수 후 단독망으로 반입한다.
...
- 현재 개발 PC에서는 wheelhouse 생성, `.build-venv` 생성, `.build-venv` 기반 패키징까지 검증했다.
- 현재 검증된 wheelhouse는 Python 3.12 / Windows x64 기준이다. 다른 Python 버전이나 OS에서는 해당 환경에 맞는 wheelhouse를 다시 생성해야 한다.
- GitHub에는 wheelhouse와 `.build-venv`를 올리지 않는다.
